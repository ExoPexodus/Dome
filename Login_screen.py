...
def account_creation():
        ...
        if len(username) == 0:
            tkinter.messagebox.showinfo("Error", "Error: username is empty")
        else:
            ...
            if cur.fetchall():
                ...
                if len(email) == 0:
                    tkinter.messagebox.showinfo("Error", "Error: Email is empty")
                else:
                    cur.execute(check_email_script)
                    if cur.fetchall():
                        tkinter.messagebox.showinfo("Error", "Error: email already exists")
                    else:
                        if len(password) <= 7:
                            tkinter.messagebox.showinfo("Error", "Error: Please make the password at least 8 characters for security reasons")
                        else:
